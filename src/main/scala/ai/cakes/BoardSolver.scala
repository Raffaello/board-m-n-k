...
object BoardSolver extends App {
  ...
  sealed trait StatsWrapper {
    def stats(): _Stats = new _Stats {
      totalCalls = ai.cakes.Stats.totalCalls
      cacheHits = ai.cakes.Stats.cacheHits
    }
  }
  ...
  sealed class MiniMaxRaw(m: Short, n: Short, k: Short) extends SolverBoardRaw(m, n, k) {
    def result(): Int = ai.cakes.old.minimax(this, isMaximizingPlayer = true)
  }

  sealed class MiniMaxTraitRaw(m: Short, n: Short, k: Short) extends SolverBoardAiTrait(m, n, k) with ai.cakes.MiniMaxRaw {
  ...
  sealed class NegaMaxRaw(m: Short, n: Short, k: Short) extends SolverBoardRaw(m, n, k) {
    def result(): Int = ai.cakes.old.negamax(this, 1)
  }
  ...
  sealed class AlphaBetaRaw(m: Short, n: Short, k: Short) extends SolverBoardRaw(m, n, k) {
    def result(): Double = ai.cakes.alphaBeta(this)
  }

  sealed class AlphaBetaTTOld(m: Short, n: Short, k: Short) extends SolverBoardOldRaw(m, n, k) with ai.cakes.old.TranspositionTable {
    def result(): old.Transposition = ai.cakes.old.alphaBetaWithMem(this, this)
  }

  sealed class AlphaBetaTT(m: Short, n: Short, k: Short) extends SolverBoardRaw(m, n, k) with ai.cakes.TranspositionTable2dArrayString {
    def result(): Transposition = ai.cakes.alphaBetaWithMem(this, this)
  }
  ...
  sealed class AlphaBetaTraitTTOld(m: Short, n: Short, k: Short) extends SolverBoardAiTrait(m, n, k) with AlphaBeta with ai.cakes.old.TranspositionTable {
    def result(): Int = this.solve
  }
