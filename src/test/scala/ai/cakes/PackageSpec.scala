...
import ai.cakes
import ai.cakes.types.{AlphaBetaStatus, AlphaBetaValues}
...
class PackageSpec extends FlatSpec with Matchers {
  ...
  def drawWithTransposition(t: TranspositionTable, g: BoardMNK): Unit = {
    it should "draw with transposition" in {
      alphaBetaWithMem(t, g) shouldBe cakes.Transposition(0, 0, AlphaBetaValues(0, Int.MaxValue), isMaximizing = true)
    }
  }
